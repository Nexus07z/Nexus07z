...
exports.runtime2 = function(mseconds) {
	mseconds = Number(mseconds*0.001);
	var d = Math.floor(mseconds / (3600 * 24));
	var h = Math.floor(mseconds % (3600 * 24) / 3600);
	var m = Math.floor(mseconds % 3600 / 60);
	var s = Math.floor(mseconds % 60);
	var dDisplay = d > 0 ? d + (d == 1 ? " día, " : " días, ") : "";
	var hDisplay = h > 0 ? h + (h == 1 ? " hora, " : " horas, ") : "";
	var mDisplay = m > 0 ? m + (m == 1 ? " minuto, " : " minutos, ") : "";
	var sDisplay = s > 0 ? s + (s == 1 ? " segundo" : " segundos") : "";
